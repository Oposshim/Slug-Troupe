...
[BepInPlugin("Slug-Troupe", "slugtroupe", "1.0.0")]
sealed class Plugin : BaseUnityPlugin
{
    ...
    private void Init(On.RainWorld.orig_OnModsInit orig, RainWorld self)
    {
        ...
        atlas ??= Futile.atlasManager.LoadAtlas("sprites/EXtemplatehead");

        if (atlas == null) {
            Logger.LogWarning("The clowns cannot found. Please reinstall the mod. co:");
        }
    }
    ...
    private void PlayerGraphics_DrawSprites(On.PlayerGraphics.orig_DrawSprites orig, PlayerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, UnityEngine.Vector2 camPos)
    {
        ...
        if (name != null && name.StartsWith("HeadA") && atlas._elementsByName.TryGetValue("EXtemplate" + name, out var element)) {
